...
function enterOnlineGame(room) {
  ...
  if (activeGameId !== room.game.gameId) {
    activeGameId = room.game.gameId || String(room.game.startedAt || "");
    finishingRoom = "";
  }
  ...
  if (restartButton) restartButton.textContent = "إعادة اللعب مع نفس الصديق";
...
window.requestOnlineRematch = async function () {
  ...
  try {
    ...
    if (!result.committed) {
      window.toast("تعذر طلب إعادة المباراة");
      return;
    }

    const room = result.snapshot.val();
    if (room.rematch?.[slot]) {
