...
function resolveMySlot(room) {
  if (!room) return "";

  if (room.host?.uid === uid) return "p1";
  if (room.guest?.uid === uid) return "p2";

  return window.onlineSlot || "";
}

function syncMySlot(room) {
  const slot = resolveMySlot(room);
...
async function normalizeOnlineGame(code, room) {
  ...
  const result = await runTransaction(
    ...
    current => {
      ...
      // لا نلمس وقت أو deadline لسؤال بدأ بالفعل.
      // نضبط 20 ثانية فقط عندما تكون الجولة بانتظار اختيار حرف.
      if (
...
function enterOnlineGame(room) {
  ...
  if (badge) {
    ...
    badge.textContent =
      "🌐 متصل — " +
      (window.onlineSlot === "p1" ? "المضيف" : "الضيف");
  }
...
window.onlinePick = async function (letter) {
  ...
  const result = await runTransaction(roomRef, room => {
    if (!room?.game) {
      rejectReason = "تعذر تحميل المباراة";
      return;
    }

    const slot =
      room.host?.uid === uid ? "p1" :
      room.guest?.uid === uid ? "p2" : "";

    if (!slot) {
      rejectReason = "تعذر تحديد اللاعب";
      return;
    }

    window.onlineSlot = slot;
    const game = room.game;

    if (game.status !== "picking") {
      rejectReason = "انتظر انتهاء السؤال الحالي";
      return;
    }
    ...
    game.msg =
      `${slot === "p1" ? room.host.name : room.guest.name} | ` +
      `${game.cat} بحرف (${letter})`;
  ...
  if (!result.committed) {
    window.toast(rejectReason || "تعذر اختيار الحرف");
    return;
  }

  const updatedRoom = result.snapshot.val();
  syncMySlot(updatedRoom);
...
window.onlineSwapLetter = async function () {
  ...
  const result = await runTransaction(roomRef, room => {
    ...
    if (game.status !== "answering" || game.turn !== slot) {
      rejectReason = "لا يمكن الاستبدال الآن";
      return;
    }

    if ((game.swaps?.[slot] || 0) <= 0) {
      rejectReason = "استخدمت الاستبدال في هذه المباراة";
      return;
    }

    const available = window.letters.filter(letter =>
      letter !== game.currentL &&
      !game.used?.[letter] &&
    ...
    if (!available.length) {
      rejectReason = "لا يوجد حرف بديل متاح";
      return;
    }

    game.swaps = game.swaps || { p1: 1, p2: 1 };
    game.swaps[slot] = Math.max(0, game.swaps[slot] - 1);
    ...
    game.deadline = Date.now() + game.timer * 1000;
    game.msg =
      `🔄 ${slot === "p1" ? room.host.name : room.guest.name} استبدل الحرف | ` +
  ...
  if (!result.committed) {
    window.toast(rejectReason || "تعذر استبدال الحرف");
    return;
  }
...
window.onlineStartTimer = function () {
  ...
  const expectedDeadline = Number(window.state.deadline);
  const expectedQuestionId =
    window.state.questionId ||
    `${window.state.turn}_${window.state.currentL}_${expectedDeadline}`;

  const tick = () => {
    const currentQuestionId =
      window.state.questionId ||
      `${window.state.turn}_${window.state.currentL}_${Number(window.state.deadline)}`;
    ...
    const remaining = Math.max(
      0,
      Math.ceil((expectedDeadline - Date.now()) / 1000)
    );
    ...
    if (
      ...
      if (typeof window.beep === "function") {
        ...
        window.beep(frequency, "sine", .13, .16);
      } else {
    ...
    if (remaining <= 0) {
      clearInterval(onlineTimer);
      lastTimerBeep = -1;
      handleTimeout(expectedQuestionId);
    }
  };

  tick();
  onlineTimer = setInterval(tick, 250);
};

async function handleTimeout(expectedQuestionId) {
  if (!roomCode) return;
  ...
  const result = await runTransaction(gameRef, game => {
    ...
    const currentQuestionId =
      game.questionId ||
      `${game.turn}_${game.currentL}_${Number(game.deadline)}`;

    // يمنع تنفيذ انتهاء الوقت مرتين لنفس السؤال.
    if (currentQuestionId !== expectedQuestionId) return;

    const expiredPlayer = game.turn;
    game.lives[expiredPlayer] =
      Math.max(0, (game.lives[expiredPlayer] || 0) - 1);
  ...
  if (result.committed) {
    const latest = (await get(roomRef)).val();
    if (latest?.game) enterOnlineGame(latest);
    return;
  }

  // الجهاز الآخر قد يكون نفّذ المهلة؛ اعرض الحالة الأحدث.
  try {
    const latest = (await get(roomRef)).val();
    if (latest?.game) enterOnlineGame(latest);
  } catch (e) {}
...
function finishOnline(winner) {
  const currentGameId =
    window.state.gameId || String(window.state.startedAt || roomCode);

  if (finishedGameId === currentGameId) return;
