...
window.onlineStartTimer = function () {
  ...
  const tick = () => {
    // إذا تغير السؤال أو deadline نوقف مؤقت السؤال القديم.
    if (
      window.state.status !== "answering" ||
      Number(window.state.deadline) !== expectedDeadline
    ) {
      clearInterval(onlineTimer);
      return;
    }
    ...
    if (remaining <= 0) {
      clearInterval(onlineTimer);
      lastTimerBeep = -1;
      handleTimeout(expectedDeadline);
    }
  };

  tick();
  onlineTimer = setInterval(tick, 300);
};

async function handleTimeout(expectedDeadline) {
  if (!roomCode || !roomRef) return;
  ...
  const result = await runTransaction(roomRef, room => {
    ...
    // لا ننفذ إلا على نفس السؤال الذي وصل إلى صفر.
    if (
      game.status !== "answering" ||
      Number(game.deadline) !== Number(expectedDeadline)
    ) return;
  ...
  if (result.committed) {
    ...
    // قد يكون الجهاز الآخر نفّذ الانتقال بالفعل؛ نجلب الحالة الأحدث.
    try {
      const latest = (await get(roomRef)).val();
